...
/**
 * 创建带状态码的 Error（适配 voicecity-player 的 handleError）
 */
...
export const txSignedRequest = async (body: any) => {
  ...
  try {
    ...
    if (!response.ok) throw new Error(`TX Signed Request HTTP ${response.status}`)
...
export const normalizeTxMusicId = (musicId: string | number): TxNormalizedMusicId => {
  ...
  if (!normalizedMusicId) {
    throw createStatusError(400, '缺少 QQ 音乐ID')
  }
...
export const txRequest = async (url: string, body: any) => {
  try {
    ...
    if (!response.ok) throw new Error(`TX Request HTTP ${response.status}`)
    return await response.json()
  } catch (error) {
